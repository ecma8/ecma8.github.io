$(document).ready(function(){
    ...
    $(document).on('change','.menu_caozuo_list',function(){
        var menu_name=$.trim($(this).parent().siblings('.menu-left').text());
    ...
    $('#add_logo').click(function(){
        ...
        if($(this).val()=="保存当前选项"){
            ...
            $('#logo_ul_list_1 li').eq(data_val).find('img').attr('src',logo_img);
            $('#logo_ul_list li').eq(data_val).find('img').attr('src',logo_img);
            $(this).val('添加');
            $(this).removeAttr('data-val');
            //$('#logo_img').attr('src','');
        }
